...
open( FASTA_OUT, ">", 'crisprs.fasta' ) or die $!;    #open or die
...
for ( my $windowStart = 0 ; $windowStart <= ( $seqLength - $windowSize ) ; $windowStart += $stepSize ) {
...
for my $last12Seq ( keys %last12Counts ) {
	...
	if ( $last12Counts{$last12Seq} == 1 ) {
		...
		print FASTA_OUT ">crispr_$crisprCount\n", "$kMerHash{$last12Seq}\n";
	}
}
